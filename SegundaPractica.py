...
class Euler(Scene):
    def construct(self):
        ...
        theta_dot, z_dot = plane.get_function_dots(lambda z: np.exp(1j * z),
        ...
        formula_box = Rectangle(width=formula.get_width() + MED_SMALL_BUFF, height=formula.get_height() + MED_SMALL_BUFF)
        formula_box.move_to(formula).set_fill(BLACK, opacity=1).set_stroke(BLUE, opacity=1, width=DEFAULT_STROKE_WIDTH / 2)
        ...
        self.play(Create(indicate_path), run_time=3)
        self.play(Uncreate(indicate_path), run_time=3)
